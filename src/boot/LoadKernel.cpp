...
EFI_STATUS LdrLoadKernelCache(MACH_O_LOADED_INFO* loadedInfo, EFI_DEVICE_PATH_PROTOCOL* bootDevicePath)
{
    ...
    __try {
        ...
        UINT8 modelNameLength                                               = strlen(modelName);
        UINT8 devicePathLength                                              = DevPathGetSize(bootDevicePath);
        CHAR8 CONST* fileName                                               = LdrpKernelPathName ? LdrpKernelPathName : LdrpKernelCachePathName;
        UINT8 fileNameLength                                                = strlen(fileName);
        ...
        UINT8 leftLength                                                    = sizeof(tempBuffer) - modelNameLength - devicePathLength;
        ...
        if(netBoot) {
            ...
            for(UINT8 i = 0; i < ARRAYSIZE(pathFormat); i ++) {
                //
                // build path
                //
...
EFI_STATUS LdrLoadRamDisk()
{
    ...
    __try {
        ...
        UINT8 allocatedLength                                               = bufferLength;
