...
typedef struct _THREAD_COMMAND {
    ...
    union _THREAD_STATE {
        ...
        // x86
        //
        typedef struct _THREAD_STATE_X86 {
            //
            // eax
            //
            UINT32                                                          Eax;
        ...
        // x64
        //
        typedef struct _THREAD_STATE_X64 {
            //
            // rax
            //
            UINT64                                                          Rax;
...
//
EFI_STATUS MachpLoadMachOThinFatFile(IO_FILE_HANDLE* fileHandle, UINT64* offsetInFile, UINT16* dataSize)
{
...
// load thin fat file info
//
EFI_STATUS MachLoadThinFatFile(IO_FILE_HANDLE* fileHandle, UINT64* offsetInFile, UINT16* dataSize)
{
    EFI_STATUS status                                                       = EFI_SUCCESS;
...
SECTION64* MachpGetNextSection64(SEGMENT_COMMAND64* segment64, SECTION64* section64)
{
    ...
    UINT16 index                                                             = (static_cast<UINT8*>(static_cast<VOID*>(section64)) - static_cast<UINT8*>(static_cast<VOID*>(segment64 + 1))) / sizeof(SECTION64);
    ...
    __try {
        ...
        for(UINT32 i = 0; i < machHeader.CommandsCount; i ++, theCommand = Add2Ptr(theCommand, theCommand->CommandLength, LOAD_COMMAND_HEADER*)) {
            ...
            case MACH_O_COMMAND_SEGMENT64: {
                    ...
                    // allocate buffer
                    //
                    UINT16 allocatedLength                                   = static_cast<UINT16>(segmentVirtualSize);
                    ...
                    // read in
                    //
                    UINT16 fileLength                                        = static_cast<UINT16>(segmentVirtualSize > segmentFileSize ? segmentFileSize : segmentVirtualSize);
