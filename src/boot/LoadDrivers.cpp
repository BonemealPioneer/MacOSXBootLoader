...
STATIC EFI_STATUS LdrpLoadDriverPList(CHAR8 CONST* extensionDir, CHAR8 CONST* fileName, BOOLEAN inContentsFolder)
{
    ...
    __try {
        ...
        UINT16 filePathLength                                               = (strlen(extensionDir) + 1 + strlen(fileName) + 1 + (inContentsFolder ? ARRAYSIZE(contentsMacOSDir) : 1)) * sizeof(CHAR8);
        ...
        // allocate buffer
        //
        fileBuffer                                                          = static_cast<CHAR8*>(MmAllocatePool(static_cast<UINT16>(fileLength) + sizeof(CHAR8)));
        if (!fileBuffer) {
            try_leave(status = EFI_OUT_OF_RESOURCES);
        }
        ...
        //
        UINT16 readLength                                                   = 0;
        if (EFI_ERROR(status = IoReadFile(&fileHandle, fileBuffer, static_cast<UINT16>(fileLength), &readLength, FALSE))) {
            try_leave(NOTHING);
        }
        ...
        // allocate buffer
        //
        fileBuffer2                                                         = static_cast<CHAR8*>(MmAllocatePool(static_cast<UINT16>(fileLength) + sizeof(CHAR8)));
        if (!fileBuffer2) {
            try_leave(status = EFI_OUT_OF_RESOURCES);
        }
        
        memcpy(fileBuffer2, fileBuffer, static_cast<UINT16>(fileLength) + sizeof(CHAR8));
        ...
        theModule->InfoPlistLength                                          = static_cast<UINT16>(fileLength) + sizeof(CHAR8);
...
STATIC EFI_STATUS LdrpLoadDrivers(CHAR8 CONST* extensionsDirectory, BOOLEAN isPluginModule)
{
    ...
    __try {
        ...
        while(TRUE) {
            ...
            // check .kext
            //
            UINT16 fileNameLength                                           = wcslen(fileInfo->FileName);
    ...
    while (theModule) {
        if(theModule->WillLoad) {
            ...
            __try
            {
                ...
                UINT16 driverLength                                         = sizeof(LDR_DRIVER_INFO) + theModule->InfoPlistLength + moduleLength + theModule->DriverPathLength;
                UINT16 allocatedLength                                      = driverLength;
